...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fullName !== '') {
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM customers WHERE full_name = ? AND customer_id != ?');
...
ob_start();
?>
<span data-spa-title="Edit Customer — DLA" hidden></span>
<?php
if (!$customer):
?>
...
<form id="addCustomerForm" data-mode="edit" class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-5" novalidate>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
    ...
    <div>
      ...
      <input type="text" name="full_name" value="<?= h($customer['full_name']) ?>" placeholder="Enter full name..." required
    ...
    <div>
      ...
      <input type="text" name="whatsapp_number" value="<?= h($customer['whatsapp_number']) ?>" placeholder="Enter whatsapp number..." pattern="[0-9]*"
             class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand">
    ...
    <div>
      ...
      <input type="text" name="country" id="countrySelect" value="<?= h($customer['country']) ?>" list="countryList" placeholder="Select or type country..."
             class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand">
    ...
    <div>
      ...
      <input type="text" name="city" id="citySelect" value="<?= h($customer['city']) ?>" data-selected="<?= h($customer['city']) ?>" list="cityList" placeholder="Select or type city..."
             class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand">
